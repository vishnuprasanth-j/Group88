...
```
Based on the benchmarking results, the for loop implementation is slightly faster than the while loop, which kinda makes sense because in a while loop, the condition must be evaluated before every iteration, accumulating a slight overhad.
...
```{r}
ratios <- fibonacci_ratio_for(100)

plot(1:100, ratios, type = "l",
...
rho(2000)
```
We get NaN , it could be because the function is being calculated with a high value. After a few trials with lower values, I observed that anything over 345 produces NaN.If we split up the denominator and numerator and calculate them individually, we receive the following:
...
```
Both the numerator and denominator individually return Inf for n=2000. R cannot divide infinite values by one another, which is why the final result is NaN

## c.
Write an implementation which can also deal with large values of $n>1000$. 

The solution to this problem is to handle large values using a log scale. We shrink the values by applying a logarithm and then exponentiate the result to convert it back to the normal scale. For this purpose, we use lgamma(). (ref: https://stackoverflow.com/questions/42821156/how-to-overcome-the-gamma-function-limitation-in-r).  
...
```
Plotting the ratio for increasing values of $n$ shows that the curve approaches approximately $0.4$. For smaller ranges of $n$, the convergence toward this value appears slower. Based on the behavior for larger $n$, it is reasonable to guess that the limit as $n \to \infty$ is about $0.4$.
...
```
   As the number of rolls increases, the running average converges towards the expected value of 3.5. This illustrates the weak law of large numbers, with more observations, our sample mean gets closer to the true population mean.
...
abline(h = 3.5, col = "red", lty = 2)
```
We  can see that all paths start with high variability but converge towards the expected value of 3.5 as the number of rolls increases. Some paths approach the expected value from above, others from below, but they all tend to get closer to 3.5 as n increases.
...
As the number of observations $n$ grows, the average absolute difference between the running mean and the expected value becomes smaller. This behavior agrees with the weak law of large numbers.At the beginning, when only a few observations are available, the running mean fluctuates quite a lot because of the randomness in small samples. However, as more die rolls are added, the aggregate behavior becomes more predictable  and centers around the expected value of 3.5.
...
while (proportion_outside >= 0.05) {
  ...
  for (m in 1:M) {
    ...
    if (n_current == batch) {
      ...
      running_averages[m, (n_current - batch + 1):n_current] <- new_sums / ((n_current - batch + 1):n_current)
    }
  }
...
```
   The loop terminated at $n = 101900$. At this point, fewer than 5% of the replications have a running mean that differs from the expected value (3.5) by more than $\epsilon = 0.01$, meaning that about 95% of the running means lie within this interval.
...
abline(h = 0, col = "red", lty = 2)
```
       The weak Law of Large Numbers does not seem to hold for the Cauchy distribution.its because the the distribution has heavier tails, those extremme values  influence the running mean, and since the Cauchy distribution has no finite expected value, the sample mean does not stabilize as the sample size increases.
...
Placing $\bar{X}_n$ inside the expectation operator and applying linearity, we get:
...
Since the sample mean is defined as $\bar{X}_n = \frac{X_1 + X_2 + \ldots + X_n}{n}$, we start with:
...
```
 ...
 The variance is highest for $n = 100$ and lowest for $n = 10000$. For a fair die, the true mean is $\mu = 3.5$, and as $n$ increases, the sample means spread less around this value.
...
Both code blocks  violate the DRY principle,large parts are repetitive and could be replaced by a loop. In particular, we could use a loop for the residual computation and fitting process in (i) and for the train-predict evaluate procedure in (ii). In the loops, hard coded values could be replaced with descriptively named variables, e.g. threshold. Descriptions can then be provided at initialization and where they are updated
## c.
