...
```{r}
population_data <- read.csv("population_data_worldbankgroup.csv", header = TRUE, sep=",", skip = 4)
...
europe_data <- data_case_study2 %>%
  filter(continent == "Europe", !is.na(expenditure), !is.na(net_migr_rate))
...
interactive_plot <- ggplot(data_case_study2, aes(x = expenditure, y = net_migr_rate, text = country)) +
  geom_point(aes(size = population_2020, color = continent), alpha = 0.7) +    # color according to continent
...
ggparcoord(data_case_study2 %>% tidyr::drop_na(),
            columns = c("expenditure", "youth_unempl_rate", "net_migr_rate"),
            groupColumn = "status",
            scale = "uniminmax",
            alphaLines=0.35,
            order = "anyClass") +
...
world_map <- joinCountryData2Map(data_case_study2, joinCode = "ISO3", nameJoinColumn = "ISO3")
